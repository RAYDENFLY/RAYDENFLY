...
app.listen(8080);
const port = (8080)
console.log(`AMS System : Save Booting Has Been Started https://localhost:${port}`)
...
app.post('/register', (req, res) => {
    ...
    const { username, password, email, firstname, lastname, date, bio } = req.body;

    // Validate the form data
    if (!username || !password || !email  || !firstname || !lastname || !date) {
      // If any of the fields are empty, render the registration page with an error message
      res.render('admin/register', { error: 'All fields are required' });
    } else {
        // Read the existing users data from the file
        let users = JSON.parse(fs.readFileSync('./users.json'));
...
app.get('/some-route', (req, res) => {
  ...
  res.render('some-view', { error: errorMessage });
});
